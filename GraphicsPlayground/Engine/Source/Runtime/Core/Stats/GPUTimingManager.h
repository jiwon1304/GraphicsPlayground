...
#include "Stats/StatDefine.h"

struct FGPUTimingSample
{
    double ElapsedTimeMs = -3.0;
    uint64 LastUpdateFrame = 0;
};

struct FGPUTimingInitParams
{
    uint32 NumBufferedFrames = 3;

    // D3D11 전용 컨텍스트 (필요 시 채움)
    void* D3DDevice = nullptr;
    void* D3DContext = nullptr;

    // OpenGL은 Initialize 시점에 GL 컨텍스트가 current여야 합니다.
};
...
public:
    ...
    virtual void StopTimestamp(const TStatId& StatId) = 0;
    virtual void EndFrame() = 0;

    virtual void RetrieveResults() = 0;

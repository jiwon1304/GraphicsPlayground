...
struct FBoneWeightMapping
{
    IndexType ControlPointIndex;
    TArray<FBoneWeight> Weights;
};

// FReferenceSkeleton has bone names in TArray format.
// To speed up the bone name to index lookup, we use TMap here.
struct FBoneNames
{
    ...
    {
        if (const int32* FoundIndex = NameToIndex.Get(BoneName))
        {
            return *FoundIndex;
        }
        return INDEX_NONE;
...
FTransform ConvertFbxMatrixToTransform(const FbxAMatrix &InMatrix)
{
    FbxVector4 T = InMatrix.GetT();
    FbxQuaternion Q = InMatrix.GetQ();
...
/**
 * Load skeleton from the given FbxScene.
 * @param OutBoneInfos Array of bone infos.
 * @param OutBoneNameToIndex Map of bone names to their indices on OutBoneInfos.
 * @param OutLocalBindPoses Array of local bind pose transforms.
 * @param OutRestBindPoses Array of rest bind pose transforms.
 * @param ExtraBindPoses Array of extra bind pose transforms.
 * @note OutBoneInfos and OutBoneNameToIndex has 1:1 mapping.
 */
void LoadSkeleton(
    TArray<FMeshBoneInfo>& OutBoneInfos,
    TMap<FString, int32>& OutBoneNameToIndex,
    FBindPoseInfo& OutLocalBindPoses,
    FBindPoseInfo& OutRestBindPoses,
    TArray<FBindPoseInfo>& ExtraBindPoses,
)
{
    assert(GFbxScene);

    const int32 PoseCount = GFbxScene->GetPoseCount();
    // First parse bones from the bind pose
    // We expect that all poses have the same bone hierarchy
    for (int32 PoseIndex = 0; PoseIndex < PoseCount; ++PoseIndex)
    {
        FbxPose* Pose = GFbxScene->GetPose(PoseIndex);
        if (Pose && Pose->IsBindPose())
        {
            const int32 NodeCount = Pose->GetCount();
            OutBoneInfos.SetNum(NodeCount);
            for (int32 NodeIndex = 0; NodeIndex < NodeCount; ++NodeIndex)
            {
                ...
                // Save this node as bone
                const FString BoneName = Node->GetName();
                FMeshBoneInfo BoneInfo;
                BoneInfo.Name = BoneName;
                if (ParentNode)
                {
                    const FString ParentBoneName = ParentNode->GetName();
                    assert(OutBoneNameToIndex.Contains(ParentBoneName) && "Parent bone must be already added");
                    BoneInfo.ParentIndex = OutBoneNameToIndex[ParentBoneName];
                }
                else
                {
                    BoneInfo.ParentIndex = INDEX_NONE;
                }
                OutBoneNameToIndex.Add(BoneName, NodeIndex);
                OutBoneInfos[NodeIndex] = BoneInfo;
            }
            break; // Only need to parse one bind pose for bone hierarchy
        }
    }

    // Parse each pose in the scene
    for (int32 PoseIndex = 0; PoseIndex < PoseCount; ++PoseIndex)
    {
        FbxPose* Pose = GFbxScene->GetPose(PoseIndex);
        if (!Pose) continue;

        FBindPoseInfo* TargetBindPoses = nullptr;
        if (Pose->IsBindPose())
        {
            // Main bind pose
            TargetBindPoses = &OutLocalBindPoses;
        }
        else if (Pose->IsRestPose())
        {
            // Rest pose
            TargetBindPoses = &OutRestBindPoses;
        }
        else
        {
            // Extra bind pose
            ExtraBindPoses.Add(BindPoseArray());
            TargetBindPoses = &ExtraBindPoses.Last();
        }

        TargetBindPoses->PoseName = Pose->GetName();

        // Parse each bone nodes in the pose
        const int32 NodeCount = Pose->GetCount();
        assert ( NodeCount == OutBoneInfos.Num() && "Pose node count must match the bone count" );
        for (int32 NodeIndex = 0; NodeIndex < NodeCount; ++NodeIndex)
        {
            FbxNode* Node = Pose->GetNode(NodeIndex);
            if (!Node) continue;
            ...
            /**
             * We will find "local" bind pose matrix with global bind pose matrices
             * of the node and its parent.
             */
            FbxAMatrix NodeGlobalMatrix = Pose->GetMatrix(NodeIndex);
            if (ParentNode)
            {
                const int32 ParentNodeIndex = Pose->Find(ParentNode);
                assert(ParentNodeIndex >= 0 && "Parent node must be found in the pose");
                FbxAMatrix ParentGlobalMatrix = Pose->GetMatrix(ParentNodeIndex);
                FbxAMatrix ParentGlobalInverseMatrix = ParentGlobalMatrix.Inverse();
                FbxAMatrix LocalBindMatrix = ParentGlobalInverseMatrix * NodeGlobalMatrix;

                TargetBindPoses->LocalBindPoses.Add(ConvertFbxMatrixToTransform(LocalBindMatrix));
            }
            else
            {
                // Node is the root bone
                TargetBindPoses->LocalBindPoses.Add(ConvertFbxMatrixToTransform(NodeGlobalMatrix));
            }
        }
    }
}
...
/**
 * Load materials from the given FbxMesh.
 * @param InMesh The FbxMesh to load materials from.
 * @param OutMaterials Array to store loaded material data.
 * @param OutTextures Array to store loaded texture data.
 * @note This function only loads material properties and texture file paths.
 * The material index is same as the material index in the FbxMesh.
 */
void LoadMaterials(
    FbxMesh* InMesh,
    TArray<FMaterialLoadData>& OutMaterials,
    TArray<FTextureLoadData>& OutTextures)
{
    const int32 materialCount = InMesh->GetNode()->GetMaterialCount();
    OutMaterials.SetNum(materialCount);
    
    for (int i = 0; i < materialCount; ++i)
    {
        FbxSurfaceMaterial* material = InMesh->GetNode()->GetMaterial(i);
        
        FMaterialLoadData& Material = OutMaterials[i];
        Material.AssetName = material->GetName();
        ...
        if (diffuse.IsValid())
        {
            ...
            Material.DiffuseColor = FVector(color[0], color[1], color[2]);
            
            FbxTexture* texture = diffuse.GetSrcObject<FbxTexture>();
            FbxFileTexture* fileTexture = FbxCast<FbxFileTexture>(texture);
            if (fileTexture)
            {
                Material.TexturePaths.Emplace(ETextureType::Diffuse, StringToWString(fileTexture->GetFileName()));
            }
        }
        ...
        if (specular.IsValid())
        {
            ...
            Material.SpecularColor = FVector(color[0], color[1], color[2]);

            FbxTexture* texture = specular.GetSrcObject<FbxTexture>();
            FbxFileTexture* fileTexture = FbxCast<FbxFileTexture>(texture);
            if (fileTexture)
            {
                Material.TexturePaths.Emplace(ETextureType::Specular, StringToWString(fileTexture->GetFileName()));
            }
        }
        ...
        if (normal.IsValid())
        {
            ...
            if (fileTexture)
            {
                Material.TexturePaths.Emplace(ETextureType::Normal, StringToWString(fileTexture->GetFileName()));
            }
        }
        ...
        if (emissive.IsValid())
        {
            ...
            Material.EmissiveColor = FVector(color[0], color[1], color[2]);
            
            FbxTexture* texture = emissive.GetSrcObject<FbxTexture>();
            FbxFileTexture* fileTexture = FbxCast<FbxFileTexture>(texture);
            if (fileTexture)
            {
                Material.TexturePaths.Emplace(ETextureType::Emissive, StringToWString(fileTexture->GetFileName()));
            }
        }

        // Alpha
        FbxProperty alpha = material->FindProperty(FbxSurfaceMaterial::sTransparentColor);
        if (alpha.IsValid())
        {
            FbxTexture* texture = alpha.GetSrcObject<FbxTexture>();
            FbxFileTexture* fileTexture = FbxCast<FbxFileTexture>(texture);
            if (fileTexture)
            {
                Material.TexturePaths.Emplace(ETextureType::Alpha, StringToWString(fileTexture->GetFileName()));
            }
        }
        ...
        if (ambient.IsValid())
        {
            ...
            Material.AmbientColor = FVector(color[0], color[1], color[2]);

            FbxTexture* texture = ambient.GetSrcObject<FbxTexture>();
            FbxFileTexture* fileTexture = FbxCast<FbxFileTexture>(texture);
            if (fileTexture)
            {
                Material.TexturePaths.Emplace(ETextureType::Ambient, StringToWString(fileTexture->GetFileName()));
            }
        }
        ...
        {
            Material.Shininess = shininess.Get<FbxDouble>();
        }

        // @todo : is it correct?
        FbxProperty metalic = material->FindProperty(FbxSurfaceMaterial::sReflectionFactor);
        if (metalic.IsValid())
        {
            Material.Metallic = metalic.Get<FbxDouble>();
        }

        FbxProperty roughness = material->FindProperty(FbxSurfaceMaterial::sBumpFactor);
        if (roughness.IsValid())
        {
            Material.Roughness = roughness.Get<FbxDouble>();
        }
    }
}
...
/**
 * Bone weights are stored by FbxSkin and FbxCluster, not by the mesh's control points.
 * This function extracts the skin weights from the FbxMesh and organizes them
 * into a mapping from control point indices to their associated bone weights.
 * @param InMesh The FbxMesh containing skinning information.
 * @param InBoneNameToIndex Map of bone names to their indices.
 * @param OutBoneWeights Output mapping from control point indices to bone weights.
 * @note We specify bone by name and look up its index using InBoneNameToIndex.
 * Therefore the bone name-to-index mapping must be consistent during the parsing.
 */
void LoadSkinWeights(
    FbxMesh* InMesh,
    const TMap<FString, int32>& InBoneNameToIndex,
    TMap<int32, TArray<BoneWeights>>& OutBoneWeights
)
{   
    int32 skinCount = InMesh->GetDeformerCount(FbxDeformer::eSkin);
    for (int32 s = 0; s < skinCount; ++s)
    {
        FbxSkin* skin = static_cast<FbxSkin*>(InMesh->GetDeformer(s, FbxDeformer::eSkin));
        int32 clustureCount = skin->GetClusterCount();
        for (int32 c = 0; c < clustureCount; ++c)
        {
            ...
            int32 boneIndex = InBoneNameToIndex[boneName];
...
uint64 HashVertex(const FSkeletalMeshVertex& Vertex)
{
    // Simple hash function combining position, normal, tangent, UV, bone indices, and bone weights
    uint64 hash = 0;
    auto hashCombine = [&hash](const void* data, size_t size)
    {
        ...
        {
            hash ^= static_cast<uint64>(bytes[i]) + 0x9e3779b9 + (hash << 6) + (hash >> 2);
        }
    };

    hashCombine(&Vertex.X, sizeof(Vertex.X));
    hashCombine(&Vertex.Y, sizeof(Vertex.Y));
    hashCombine(&Vertex.Z, sizeof(Vertex.Z));
    hashCombine(&Vertex.NormalX, sizeof(Vertex.NormalX));
    hashCombine(&Vertex.NormalY, sizeof(Vertex.NormalY));
    hashCombine(&Vertex.NormalZ, sizeof(Vertex.NormalZ));
    hashCombine(&Vertex.TangentX, sizeof(Vertex.TangentX));
    hashCombine(&Vertex.TangentY, sizeof(Vertex.TangentY));
    hashCombine(&Vertex.TangentZ, sizeof(Vertex.TangentZ));
    hashCombine(&Vertex.TangentW, sizeof(Vertex.TangentW));
    hashCombine(&Vertex.U, sizeof(Vertex.U));
    hashCombine(&Vertex.V, sizeof(Vertex.V));
    for (int i = 0; i < 8; ++i)
    {
        hashCombine(&Vertex.BoneIndices[i], sizeof(Vertex.BoneIndices[i]));
        hashCombine(&Vertex.BoneWeights[i], sizeof(Vertex.BoneWeights[i]));
    }

    return hash;
}

// Calculate tangent for the given pivot vertex using its two adjacent vertices.
void CalculateTangent(FSkeletalMeshVertex& PivotVertex, 
    const FSkeletalMeshVertex& Vertex1, const FSkeletalMeshVertex& Vertex2)
{
...
bool LoadStaticMesh(
    FbxMesh* InMesh,
    FStaticMeshLoadData& OutStaticMesh)
{
    assert(InMesh);
     ...
     */
    FStaticMeshLoadData& MeshData = OutStaticMesh;
    MeshData.AssetName = InMesh->GetName();
    
    int polygonCount = InMesh->GetPolygonCount();
    FbxVector4* controlPoints = InMesh->GetControlPoints();
    FbxLayerElementNormal* normalElement = InMesh->GetElementNormal();
    FbxLayerElementTangent* tangentElement = InMesh->GetElementTangent();
    FbxLayerElementUV* uvElement = InMesh->GetElementUV();
    FbxLayerElementMaterial* materialElement = InMesh->GetElementMaterial();

    TMap<uint64, uint32> indexMap;
    ...
    for (int polygonIndex = 0; polygonIndex < polygonCount; ++polygonIndex)
    {
        for (int vertexIndex = 0; vertexIndex < 3; ++vertexIndex)
        {
            ...
            v.TangentX = tangent[0];
            v.TangentY = tangent[1];
            v.TangentZ = tangent[2];
            v.TangentW = tangent[3];
            ...
            if (materialElement)
            {
                ...
                int materialIndex = indices.GetAt(polygonIndex);
                materialIndexToIndices[materialIndex].Add(static_cast<uint32>(controlPointIndex));
            }
            
            // indices process
            uint64 hash = HashVertex(v);
            if (indexMap.Contains(hash))
            {
                uint32 index = indexMap[hash];
                MeshData.Indices.Add(index);
            }
            else
            {
                uint32 newIndex = static_cast<uint32>(MeshData.vertices.Num());
                MeshData.vertices.Add(v);
                MeshData.VerticesPositionOnly.Add({v.X, v.Y, v.Z});
                MeshData.Indices.Add(newIndex);
                indexMap.Add(hash, newIndex);
            }
        }
    }

    // subset 처리.
    // Make sure that the number of materials is same as FMaterialLoadData array.
    uint32 accumIndex = 0;
    for (auto& [materialIndex, indices]: materialIndexToIndices)
    {
        FStaticSubMeshInfo SubMeshInfo;
        SubMeshInfo.IndexStart = accumIndex;
        SubMeshInfo.IndexCount = static_cast<uint32>(indices.Num());
        SubMeshInfo.MaterialIndex = materialIndex;
        MeshData.SubMeshes.Add(SubMeshInfo);
        accumIndex += SubMeshInfo.IndexCount;
    }
    ...
    if (tangentElement == nullptr)
    {
        ...
        {
            FStaticMeshVertex& Vertex0 = MeshData.Vertices[MeshData.Indices[i]];
            FStaticMeshVertex& Vertex1 = MeshData.Vertices[MeshData.Indices[i + 1]];
            FStaticMeshVertex& Vertex2 = MeshData.Vertices[MeshData.Indices[i + 2]];
...
/**
 * Load vertices, materials, and textures from the given FbxMesh.
 * @param Mesh The FbxMesh to load. Assumes Mesh->GetDeformerCount(FbxDeformer::eSkin) > 0
 * @param InBoneNameToIndex Map of bone names to their indices. This will be used for vertex skinning.
 * @param OutSkeletalMesh Skeletal mesh load data.
 * @note Material should be mapped to Submesh after this function call.
 */
bool LoadSkeletalMesh(
    FbxMesh* InMesh,
    TMap<FString, int32>& InBoneNameToIndex,
    FSkeletalMeshLoadData& OutSkeletalMesh)
{
    assert (InMesh);

    // Prepare skin weights
    TMap<int32, TArray<BoneWeights>> boneWeight;
    LoadSkinWeights(InMesh, InBoneNameToIndex, boneWeight);

    /**
     * Parse vertex & index data
     */
    FSkeletalMeshLoadData& MeshData = OutSkeletalMesh;
    MeshData.AssetName = InMesh->GetName();
    ...
    for (auto& [materialIndex, indices]: materialIndexToIndices)
    {
        FSkeletalSubMeshInfo SubMeshInfo;
        SubMeshInfo.IndexStart = accumIndex;
        SubMeshInfo.IndexCount = static_cast<uint32>(indices.Num());
        SubMeshInfo.MaterialIndex = materialIndex;
        MeshData.SubMeshes.Add(SubMeshInfo);
    ...
    if (tangentElement == nullptr)
    {
        for (int i = 0; i + 2 < MeshData.Indices.Num(); i += 3)
        {
            FSkeletalMeshVertex& Vertex0 = MeshData.Vertices[MeshData.Indices[i]];
            FSkeletalMeshVertex& Vertex1 = MeshData.Vertices[MeshData.Indices[i + 1]];
            FSkeletalMeshVertex& Vertex2 = MeshData.Vertices[MeshData.Indices[i + 2]];
            CalculateTangent(Vertex0, Vertex1, Vertex2);
            CalculateTangent(Vertex1, Vertex2, Vertex0);
            CalculateTangent(Vertex2, Vertex0, Vertex1);
        }
